...
class SIGNETGAME_API UTargetingComponent : public UActorComponent
{
	GENERATED_BODY()
...
public:
	...
	bool bAllowControlRotation = false;
	
	// The minimum distance to enable target locked on.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Target System")
	float MinimumDistanceToEnable = 1200.0f;

	// The minimum distance to the target to continue updating the control rotation.
	// If the distance is less than this value, the control rotation will not be updated to avoid spinning.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Target System")
	float MinimumDistanceToUpdateRotation = 150.0f;
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Target System")
	float StartRotatingThreshold = 0.5f;
	...
	// Only used when Sticky Target is enabled.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Target System|Sticky Feeling on Target Switch")
	float StickyRotationThreshold = 10.0f;
...
private:
	...
	bool GetCameraLocationAndRotation(FVector& OutLocation, FRotator& OutRotation) const;

	FVector GetTargetLocation(const AActor* OtherActor) const;
